...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):   
    ...
    #
    # The following code refers to the lecture videos.
    dim = 2 * (1 + num_landmarks)  # dimension of the matrix
    ...
    Xi = matrix()
    Xi.zero(dim, 1)  # dimension of the matrix
    Xi.value[0][0] = world_size / 2.0
    Xi.value[1][0] = world_size / 2.0

    # process the data
    for k in range(len(data)):  # for each time step
        measurement = data[k][0]
        motion = data[k][1]

        # integrate the measurements
        for i in range(len(measurement)):
            m = 2 * (measurement[i][0] + 1)  # landmark index

            # update the information matrix/vector
            for b in range(2):
        ...
        # expand Omega and Xi to include new pose and landmark(s)
        idxs = [0, 1] + list(range(4, dim + 2))
        Omega = Omega.expand(dim + 2, dim + 2, idxs, idxs)
        Xi = Xi.expand(dim + 2, 1, idxs, [0])

        # update the information matrix/vector with the motion
        for b in range(4):
